...
6. **차수(Round)가 최소 거래 단위** — 예치·취소·락·반납·정산·환급 모두 차수별로 분개. 모임/동아리는 집계.
   - 기간 제한형 에스크로: 신청 마감 후 시간별 취소 위약금(마감~24h전 0%·24h~6h 50%·6h~만남 100%, 위약금은 그룹 공동비용 충당·플랫폼/총무 귀속 0; 총무 출금 잠김) → 만남시각 락(취소 차단·출금 활성) → 총무 출금(정산 타이머) → OCR 증빙·잔액 반납 → 자동 균등 정산·현금화. 상세 `docs/05`.
